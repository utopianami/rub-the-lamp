...
| **oh-my-claudecode** | Claude Code 다중 에이전트 | 의도 감지 → 전문가 자동 위임 (architect/executor/designer 등) |
...
- **역할 분리**: Claude = 코딩, Cursor = 리뷰 (각자 전문 영역)
- **교차 검증**: 다른 모델이 blind spot과 edge case 검증
...
- **28개 전문 에이전트**: 작업 의도를 감지해서 적절한 전문가에게 자동 위임
- **병렬 처리**: 복잡한 작업을 동시에 처리
- **3단계 모델**: Haiku(빠름) / Sonnet(균형) / Opus(정밀) 자동 선택

### 서브에이전트 예시

| 요청 | 위임되는 에이전트 | 역할 |
|-----|-----------------|------|
| "이 버그 원인 분석해줘" | `architect` | 복잡한 디버깅, 아키텍처 분석 |
| "로그인 기능 구현해줘" | `executor` | 코드 작성, 기능 구현 |
| "대시보드 UI 만들어줘" | `designer` | UI/UX, 프론트엔드 컴포넌트 |
| "이 라이브러리 사용법 찾아봐" | `researcher` | 외부 문서, API 조사 |
